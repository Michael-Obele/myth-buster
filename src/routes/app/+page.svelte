...
<div class="min-h-screen bg-gradient-to-br from-background to-background/50">
  ...
  <header class="sticky top-0 z-10 flex w-full items-center justify-between border-b bg-background/80 p-4 backdrop-blur-sm lg:hidden">
    <h1 class="flex items-center gap-2 text-xl font-bold">
      ...
      <span class="text-white">Myth<span class="text-primary">Buster</span></span>
    </h1>
  ...
  <!-- Mobile Menu -->
  <div class="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm transition-all duration-100 md:hidden" class:block={showMobileMenu} class:hidden={!showMobileMenu}>
    <div class="fixed left-0 top-0 h-full w-[250px] border-r bg-background p-4 shadow-lg transition-transform duration-300 md:-translate-x-full" class:translate-x-0={showMobileMenu} class:-translate-x-full={!showMobileMenu}>
                ...
                <Accordion.Content>
                  <div class="mt-4 space-y-3">
                    {#each form.data.citations as citation, i}
                      <div class="flex items-start gap-3 rounded-md border border-primary/20 bg-muted/30 p-3">
